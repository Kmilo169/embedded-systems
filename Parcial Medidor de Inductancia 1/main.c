...
volatile uint32_t p1=0,p2=0,of=0,pasos=0,fs=0;
volatile unsigned long il=0,pruebita=0.00001;
/* USER CODE END PV */
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, RW_Pin|EN_Pin|D4_Pin|D5_Pin
                          |D6_Pin|D7_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, RS_Pin|PULSO_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : RS_Pin PULSO_Pin */
  GPIO_InitStruct.Pin = RS_Pin|PULSO_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
void clean_row(char row)
{
	char casillas=0;
	lcd_put_cur(row, 0);
	for(casillas;casillas<16;casillas++)
	{
		fx(0x80,1);
	}
}
